```
print("Welcome to my github!")
```
---
# Oi, sou Henry 👋

Estudante de Informática no IFSP e desenvolvedor em formação.  
Interesse em **programação, automação e desenvolvimento de software**.

Atualmente estudando e desenvolvendo projetos com **Python**, buscando transformar aprendizado em projetos práticos.

---

## 🛠 Tecnologias & Ferramentas

<img src="https://skillicons.dev/icons?i=html,css,js,python,c,,"/>
</p>

---

## 🚀 Atualmente

- 🎓 Estudante de **Informática no IFSP**
- 💻 Aprendendo **desenvolvimento de software**
- 📚 Estudando **Python, JavaScript e C**
- 🧠 Desenvolvendo projetos para evoluir na programação

---

## 📌 Projetos em destaque

| Projeto | Descrição | Tech |
|-------|-----------|------|
| python-estudos | Exercícios e desafios de lógica e programação | Python |
| algoritmos-c | Implementações de algoritmos e estruturas básicas | C |
| projetos-js | Pequenos projetos e práticas com JavaScript | JavaScript |

---

## 📜 Formação

- 🎓 Técnico em Informática — **IFSP**
- 📚 Estudos em **Programação e Desenvolvimento de Software**

---

## 📬 Contato

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=for-the-badge&logo=linkedin)](https://linkedin.com/in/henrydesilva)
---
